...
### Текущие проблемы
Во время загрузки map_1.bmp ломается путь, выводится ошибка "LoadBMP Load Error: 5", обозначающая неправильную шапку файла. Однако с шапкой всё в порядке.
Некорректно обробатывается ввод координат пользователя, ложно выставляя за диапазон карты.
Некоректно считывается csv файл с остановками, добавляется две лишних с нулевыми координатами
Некоректно производится инициализация структуры map, из-за чего освобождение памятия вызывает segmentation fall.
